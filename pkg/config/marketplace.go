...
func BuiltInMarketplace() []MarketplaceItem {
	return []MarketplaceItem{
		...
		},
		{
			...
			RequiredEnv: []string{"GITLAB_TOKEN"},
			Tags:        []string{"git", "mr", "ci"},
		},
		{
			Name:        "sentry",
			Type:        "mcp-server",
		...
		},
		{
			...
			MCPConfig: &MCPServerConfig{
				...
				Env:     map[string]string{"NOTION_API_KEY": "${NOTION_API_KEY}"},
			},
		...
		},
		{
			Name:        "puppeteer",
			Type:        "mcp-server",
			Description: "Puppeteer — browser automation, screenshots, web scraping",
			Category:    "data",
			Source:      "modelcontextprotocol/servers (official)",
			Harnesses:   []string{"claude"},
			MCPConfig: &MCPServerConfig{
			...
			Tags: []string{"browser", "scraping", "automation", "testing"},
		},
		{
			Name:        "memory",
			Type:        "mcp-server",
